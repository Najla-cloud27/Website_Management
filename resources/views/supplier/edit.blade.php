<x-app-layout>
    ...
    <div class="mx-auto max-w-2xl space-y-6 px-4 py-6 sm:px-6 lg:px-8">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-2xl font-bold tracking-tight text-slate-800">Edit Supplier</h2>
                <p class="text-sm text-slate-500">Perbarui data supplier <strong>{{ $supplier->nama_supplier }}</strong>.</p>
            </div>
        </div>
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <form action="{{ route('supplier.update', $supplier->id) }}" method="POST" class="space-y-5">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <textarea name="alamat"
                              ...
                              class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm text-slate-800 shadow-sm placeholder:text-slate-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 focus:outline-none">{{ old('alamat', $supplier->alamat) }}</textarea>
                    @error('alamat')
                        <p class="mt-1.5 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
