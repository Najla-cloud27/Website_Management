<x-app-layout>
    ...
    <div class="mx-auto max-w-2xl space-y-6 px-4 py-6 sm:px-6 lg:px-8">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-2xl font-bold tracking-tight text-slate-800">Edit Pengguna</h2>
                <p class="text-sm text-slate-500">Perbarui data akun <strong>{{ $user->name }}</strong>.</p>
            </div>
        </div>
            ...
            <form action="{{ route('users.update', $user->id) }}" method="POST" class="space-y-5">
                ...
                <div>
                    ...
                    <select name="role"
                        ...
                        <option value="user" @selected(old('role', $user->role) == 'user')>User - anggota tim (menu standar)</option>
                        <option value="admin" @selected(old('role', $user->role) == 'admin')>Admin - akses penuh</option>
                    </select>
                    @if ($user->id == auth()->id())
                        <p class="mt-1.5 text-xs text-slate-400">Anda tidak dapat mencabut role admin dari akun sendiri.</p>
                    @endif
                    @error('role')
                        <p class="mt-1.5 text-sm text-red-600">{{ $message }}</p>
                    @enderror
