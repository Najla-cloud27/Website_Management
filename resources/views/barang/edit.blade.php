<x-app-layout>
    ...
    <div class="mx-auto max-w-2xl space-y-6 px-4 py-6 sm:px-6 lg:px-8">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-2xl font-bold tracking-tight text-slate-800">Edit Barang</h2>
                <p class="text-sm text-slate-500">Perbarui data barang <strong>{{ $barang->nama_barang }}</strong>.</p>
            </div>
        </div>
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <form action="{{ route('barang.update', $barang->id) }}" method="POST" class="space-y-5">
                    ...
                    <div>
                        ...
                        <select name="category_id"
                            ...
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}" @selected(old('category_id', $barang->category_id) == $category->id)>
                    ...
                    <div>
                        ...
                        <select name="supplier_id"
                            ...
                            @foreach ($suppliers as $supplier)
                                <option value="{{ $supplier->id }}" @selected(old('supplier_id', $barang->supplier_id) == $supplier->id)>
                ...
                <div class="grid gap-5 sm:grid-cols-3">
                    <div>
                        <label for="stok" class="mb-1.5 block text-sm font-medium text-slate-700">Stok Saat Ini <span class="text-red-500">*</span></label>
                        <input type="number"
                               name="stok"
                               id="stok"
                               ...
                               min="0"
                               class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm text-slate-800 shadow-sm placeholder:text-slate-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 focus:outline-none">
                        <p class="mt-1.5 text-xs text-slate-400">Untuk perubahan stok harian gunakan menu Stok Masuk / Stok Keluar.</p>
                        @error('stok')
                            <p class="mt-1.5 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div>
                    ...
                    <textarea name="deskripsi"
                              ...
                              class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm text-slate-800 shadow-sm placeholder:text-slate-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 focus:outline-none">{{ old('deskripsi', $barang->deskripsi) }}</textarea>
                    @error('deskripsi')
                        <p class="mt-1.5 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
